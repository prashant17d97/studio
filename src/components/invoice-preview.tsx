...
export default function InvoicePreview({ data }: InvoicePreviewProps) {
  ...
  const renderPage = (chunk: typeof items, pageIndex: number, isLastPage: boolean) => (
    <Card key={pageIndex} className="w-[210mm] h-[297mm] mx-auto bg-white shadow-lg print-page flex flex-col relative overflow-hidden font-sans">
      <HeaderWave className="absolute top-0 left-0 w-full" />
      <header className="flex justify-between items-start pt-[23px] px-[29px] z-10 relative">
      ...
      <main className="flex-1 flex flex-col px-[24px] z-10 mt-[89px] pb-[90px] relative">
      ...
      <FooterWave className="absolute bottom-0 left-0 w-full z-0" />
      <footer className="absolute bottom-0 left-0 w-full p-4 text-white z-20">
        <div className="flex flex-col gap-1 relative z-30">
          <div className="flex items-center gap-1">
            ...
            <span className='text-[10px] font-medium text-[#E9EAEB] leading-none pt-[2px]'>{from.phone}</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3.5 h-3.5 rounded-full bg-white/10 flex items-center justify-center shrink-0">
              <Mail size={8} />
            </div>
            <span className='text-[10px] font-medium text-[#E9EAEB] leading-none pt-[2px]'>{from.email}</span>
          </div>
        </div>
      </footer>
    </Card>
